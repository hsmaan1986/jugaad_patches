...
jQuery("#startCalculators").click(function (e) {
  ...
  sendData = jQuery(
    ...
    .get();
  jQuery.get(new URL(this.href), sendData).done(function (data) {
...
function prepareForms(currentIndex, newIndex, returnData, wizard, $form) {
  ...
  switch (formId) {
    ...
    case "patient_screening_tool":
      var formValidate = jQuery("#patient_screening_tool").validate({
        errorPlacement: function (error, element) {
          jQuery(element)
            .parents(".calculator__form-group")
            .addClass("error");
  ...
  var dataStore = (function () {
    console.log(action, globalUrl, values);
    jQuery.post(new URL(action, globalUrl), values).done(function (data) {
      console.log(data);
      formData = data;
    });
    return {
  ...
  if (
    ...
    jQuery
      ...
      .done(function (data) {
        ...
        if (formDataId == "patient_screening_tool") {
